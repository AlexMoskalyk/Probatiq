...
export function NewQuestionClientPage(props: Props) {
  const { jobInfo } = props;
  const [status, setStatus] = useState<Status>("init");
  const [answer, setAnswer] = useState<string | null>(null);
  const [questionId, setQuestionId] = useState<string | null>(null);
  const currentDifficultyRef = useRef<QuestionDifficulty | null>(null);
  ...
  const {
    ...
    onFinish: (_, text) => {
      ...
      if (difficulty != null) {
        createQuestion({ text, jobInfoId: jobInfo.id, difficulty }).then(
          (result) => {
            if (!result.error) setQuestionId(result.id);
          },
        );
      }
    },
